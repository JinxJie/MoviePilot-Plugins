...
| 插件诊所 | [`plugins.v2/pluginclinic`](plugins.v2/pluginclinic) | 1.0.0 | 扫描异常与残留，逐项确认或一键清理 |
...
<details>
<summary>3. 🧹 插件诊所（PluginClinic）</summary>

**v1.0.0 · 插件诊断清理 · MoviePilot 插件健康管理**

**功能：**
扫描并清理 MoviePilot 中无法加载的插件与卸载残留，逐项确认后清理或一键清理异常项，让重新安装一路顺畅。

**标签：**
插件诊断、残留清理、手动操作、记录留痕、消息通知

**特点：**
- 🔍 打开插件页面自动扫描，无需任何配置
- 📅 可选定时扫描：发现问题只发送通知，不自动清理
- ❌ 定位无法加载的插件（缺依赖 / 代码错误 / 目录缺失），显示具体异常
- 🗑️ 定位卸载残留（目录 / 数据 / 配置 / 模块缓存）
- ✅ 每项显示具体问题，右侧「清理此项」逐项确认并执行
- 🧹 一键清理全部异常与残留项
- 📋 清理记录展示在插件页面（最近 20 条）
- 🛡️ 安全护栏：自身与「排除列表」中的插件永不清理

**使用说明：**
1. 安装并打开插件页面，扫描自动完成。
2. 可在设置中开启「定时扫描」，按 Cron 定期检查；发现问题只发送通知，不执行清理。
3. 确认「扫描结果」中的异常/残留插件，执行清理（页面按钮 / 命令 `/pluginclinic` / API）。
4. 清理后到插件市场重新安装即可一路顺畅。
5. 如需保护某插件，加入「排除插件 ID」配置。

**免责声明：**
清理为不可逆操作（删除插件目录、数据、配置），请确认后执行；插件诊所不会清理自身、排除列表中的插件及状态正常的插件。因误操作产生的任何问题，开发者不承担责任。

**更新说明：**
- v1.0.0：首发版本，支持自动扫描、逐项确认清理、一键清理、定时扫描告警、清理记录与安全排除。

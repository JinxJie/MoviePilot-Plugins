...
| NexusPHP 魔力兑换 | [`plugins.v2/bonusmagic`](plugins.v2/bonusmagic) | 1.0.0 | 站点操作中心 + Adapter 解析魔力商店，按策略自动兑换上传/下载量 |
...
<details>
<summary>5. 💎 NexusPHP 魔力兑换（BonusMagic）</summary>

**v1.0.0 · 站点自动化 · NexusPHP 魔力商店**

**功能：**
读取 MoviePilot 已配置的 PT 站点 Cookie 与 URL，由站点适配器动态解析魔力商店项目与价格，按策略自动兑换上传/下载量。核心调度与架构解耦，首期支持 NexusPHP。支持自适应限速、多站点并发、单站串行和安全熔断。

**标签：**
站点自动化、魔力兑换、定时任务、数据统计、消息通知

**特点：**
- 📡 首页站点操作中心：单站 / 批量 / 智能推荐三模式；支持勾选筛选与一键智能兑换（站间并发、站内限速）
- 🍪 自动读取 MoviePilot 站点管理中的 URL / Cookie / UA，无需重复填账号
- 🔍 动态解析兑换项与所需魔力，解析不到价格禁止兑换
- ⬆️⬇️ 上传/下载兑换独立开关，档位可选最便宜 / 单次最大 / 性价比最高
- 📐 策略：保留余额后最大化、最大化、固定次数；可按分享率决定优先兑上传还是下载
- ⏱️ 识别「请等待 X 秒」等限制，按站点分别学习间隔，无限制站点不强制等 10 秒
- 🧵 多站点并发，同一站点内部严格串行
- 🛡️ 单站次数上限、单次消耗上限、最低保留魔力；登录失效 / 魔力不足只停当前站
- 🧩 统一 Adapter 接口：自动识别架构，NexusPHP 为首期实现；站点特殊规则可单独覆盖
- 🖥️ 默认使用系统代理
- 📊 Lite 兼容统计网格：累计次数、消耗魔力、获得上传/下载

**使用说明：**
1. 在 MoviePilot 站点管理中启用 NexusPHP 站点并保存 Cookie。
2. 安装本插件，打开「启用插件」，按需配置上传/下载开关、保留魔力和定时。
3. 打开插件首页「站点操作中心」，点「刷新」拉取魔力与兑换项（打开页面不会自动打站）。
4. 三种入口：勾选一站「立即兑换」；勾选多站「批量兑换」；或点「智能推荐」确认后执行 / 直接「一键智能兑换」。也可定时任务 / 命令兑换。
5. 架构默认「自动识别」。个别站点路径不同时，可在「站点特殊规则」按域名覆盖。
6. 建议保持「使用系统代理」开启。

**免责声明：**
本插件仅通过站点现有页面提交兑换请求，不会修改站点其它数据。站点规则、页面结构或风控策略变化可能导致功能异常或账号风险，请自行评估后使用；因使用本插件产生的任何问题，开发者不承担责任。

**更新说明：**
- v1.0.0：首发版本，支持读取 MoviePilot 站点、Adapter 自动识别架构（首期 NexusPHP）、动态解析魔力商店、固定/最大化/保留余额与分享率优先策略、自适应限速学习、多站点并发、单站串行、安全熔断与任务统计。
